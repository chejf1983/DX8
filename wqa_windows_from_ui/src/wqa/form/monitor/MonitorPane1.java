...
public class MonitorPane1 extends javax.swing.JPanel {
    ...
    public MonitorPane1(MonitorPaneDesk parent, DevMonitor dev) {
        this.parent = parent;
        this.dev = dev;
        this.data_vector = new DataVector();
    ...
    private void UpdateComboBox() {
        ...
        //初始化曲线下拉框
        for (String name : data_vector.GetSupportDataName()) {
    ...
    final DevMonitor dev;
    private final ReentrantLock dsiplay_lock = new ReentrantLock(true);

    private void initDevice() {

        Lable_Title.setText("CH" + (dev.index + 1) + "_?" + "nm");
        //初始化数据刷新监听
        this.dev.DataEvent.RegeditListener(new EventListener<SDisplayData>() {
            @Override
        ...
        this.dev.GetParent().StateChange.RegeditListener(new EventListener<ControlState>() {
            @Override
            public void recevieEvent(Event<ControlState> event) {
                if (ControlState.DISCONNECT == event.GetEvent()) {
    ...
    private void UpdateData(SDisplayData data) {
        ...
        try {
            //刷新数据列表
            Lable_Title.setText("CH" + (dev.index + 1) + "_" + data.nm + "nm");
            this.data_vector.InputData(data);
            this.chartPane1.PaintMainLine(this.data_vector.GetdateTimeSeries(), this.data_vector.GetDataTimeDescribe());
            ChangeState(this.dev.currentState);
        } finally {
            dsiplay_lock.unlock();
        }
    }
    ...
    private void initComponents() {
        ...
        Lable_Title.setFont(new java.awt.Font("宋体", 1, 14)); // NOI18N
        ...
        Button_Config.setFont(new java.awt.Font("宋体", 0, 18)); // NOI18N
        Button_Config.setText("REF");
        ...
        Body_PaneLayout.setHorizontalGroup(
            ...
            .addGroup(Body_PaneLayout.createSequentialGroup()
                ...
                .addGroup(Body_PaneLayout.createParallelGroup(javax.swing.GroupLayout.Alignment.LEADING)
                    .addGroup(Body_PaneLayout.createSequentialGroup()
                        .addComponent(Button_Config, javax.swing.GroupLayout.PREFERRED_SIZE, 184, javax.swing.GroupLayout.PREFERRED_SIZE)
                        .addPreferredGap(javax.swing.LayoutStyle.ComponentPlacement.RELATED)
                        .addComponent(Button_ChartSwitch, javax.swing.GroupLayout.DEFAULT_SIZE, javax.swing.GroupLayout.DEFAULT_SIZE, Short.MAX_VALUE))
...
//        this.setBackground(new Color(0,0,0,0));
    ...
    private void ChangeState(DevMonitor.State st) {
        String png = "/wqa/form/monitor/resource/m_connect.png";
        switch (st) {
            case WORKING:
...
//                png = "/wqa/form/monitor/resource/warning_24p.png";
                Lable_Title.setForeground(Color.YELLOW);
                break;
            default:
...
//        Label_AlarmInfo.setIcon(new javax.swing.ImageIcon(getClass().getResource(png)));
    ...
    private void Button_delActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_Button_delActionPerformed
        if (ConfirmBox.ShowConfirmBox(MainForm.main_parent, "是否删除该设备?")) {
            WQAPlatform.GetInstance().GetManager().DeleteDevControl(this.dev.GetParent());
        }
